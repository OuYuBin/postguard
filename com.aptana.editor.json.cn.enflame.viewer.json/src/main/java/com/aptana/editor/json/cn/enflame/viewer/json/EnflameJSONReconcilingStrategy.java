...
import com.aptana.editor.common.text.reconciler.JSONFoldingComputer;
 ...
 *
 */
public class EnflameJSONReconcilingStrategy implements IReconcilingStrategy, IReconcilingStrategyExtension,
	...
	public EnflameJSONReconcilingStrategy(ISourceViewer viewer) {
		fViewer = viewer;
	}

	public EnflameJSONReconcilingStrategy(AbstractThemeableEditor editor) {
		fEditor = editor;
		if (fEditor != null)
	...
	protected IFoldingComputer createFoldingComputer(IDocument document) {
		return new EnflameJSONFoldingComputer(document);
	...
	protected void calculatePositions(boolean initialReconcile, IProgressMonitor monitor, IParseRootNode ast) {
		...
		// If we had all positions we shouldn't probably listen to cancel, but we may
		// have exited emitFoldingRegions
	...
	protected void updatePositions() {
		...
		synchronized (fPositionsLock) {
			// Create a copy to pass to updateFoldingStructure, as it may take more time
			// there.
	...
	public void updateFoldingStructure(Map<ProjectionAnnotation, Position> annotations) {
		{
			...
			if (fViewer instanceof ProjectionViewer) {
				ProjectionAnnotationModel currentModel = ((ProjectionViewer) fViewer).getProjectionAnnotationModel();
				...
				if (annotations.size() != 0 || deletions.size() != 0) {
					currentModel.modifyAnnotations(deletions.toArray(new Annotation[deletions.size()]), annotations,
							null);
				}
			}
		}
	...
	private void reconcile(boolean initialReconcile, boolean force) {
		...
		if (editor != null && editor.isFoldingEnabled()) {
			...
			ParseResult result;
			try {
				result = ParserPoolFactory.parse(IJSONConstants.CONTENT_TYPE_JSON, getDocument().get());
...
//			}
	...
	/**
	 * Runs through the {@link IBuildParticipant}s that apply to this editor's
	 * underlying file.
	 * 
	 * @param monitor
	 */
	private void runParticipants(IProgressMonitor monitor) {
		...
		List<IBuildParticipant> participants = getBuildParticipantManager()
				.getBuildParticipants(editor.getContentType());
	...
	/**
	 * Reports problems found in reconcile to the annotation model so we can draw
	 * them on the editor without creating markers on the underlying resource.
	 * 
	 * @param context
	 * @param monitor
	 */
