...
import com.keliangtek.simulab.target.editor.pages.SimulabTargetFormPage;
import com.keliangtek.simulab.target.provider.TargetItemProviderAdapterFactory;

/**
 * This is an example of a Target model editor. <!-- begin-user-doc --> <!--
 * end-user-doc -->
 * 
 * @generated NOT
 */
public class SimulabTargetEditor extends SimulabCommonFormEditor
		// MultiPageEditorPart
		implements ISimulabCommonFormEditor, IEditingDomainProvider, ISelectionProvider, IMenuListener,
		IViewerProvider {
	/**
	 ...
	 * @generated
	 */
	public static final List<String> FILE_EXTENSION_FILTERS = prefixExtensions(TargetModelWizard.FILE_EXTENSIONS, "*.");
		...
		public void partActivated(IWorkbenchPart p) {
			if (p instanceof ContentOutline) {
				if (((ContentOutline) p).getCurrentPage() == contentOutlinePage) {
					getActionBarContributor().setActiveEditor(SimulabTargetEditor.this);
			...
			} else if (p == SimulabTargetEditor.this) {
				handleActivate();
			}
		}
	...
	protected void handleActivate() {
		...
		if (!removedResources.isEmpty()) {
			if (handleDirtyConflict()) {
				getSite().getPage().closeEditor(SimulabTargetEditor.this, false);
			} else {
	...
	protected void updateProblemIndication() {
		if (updateProblemIndication) {
			BasicDiagnostic diagnostic = new BasicDiagnostic(Diagnostic.OK, "com.keliangtek.simulab.target.editor", 0,
			...
			if (lastEditorPage >= 0 && getEditor(lastEditorPage) instanceof ProblemEditorPart) {
				...
				try {
					...
					TargetEditorPlugin.INSTANCE.log(exception);
				}
			}
		}
	}
	...
	protected void setPartName(String partName) {
		int index = partName.lastIndexOf(".tfg");
		if (index > 0) {
			String name = partName.substring(0, partName.lastIndexOf(".tfg"));
			Object[] targets = SimulabTargetCore.loadSimulabTargets().stream()
					.filter(simulabTarget -> name.equals(simulabTarget.getName())).toArray();
			if (targets.length > 0) {
				partName = ((ISimulabTarget) targets[0]).getName();
			}
		}
	...
	public Diagnostic analyzeResourceProblems(Resource resource, Exception exception) {
		...
		if (hasErrors || !resource.getWarnings().isEmpty()) {
			BasicDiagnostic basicDiagnostic = new BasicDiagnostic(hasErrors ? Diagnostic.ERROR : Diagnostic.WARNING,
					"com.keliangtek.simulab.target.editor", 0,
					getString("_UI_CreateModelError_message", resource.getURI()),
					new Object[] { exception == null ? (Object) resource : exception });
			basicDiagnostic.merge(EcoreUtil.computeDiagnostic(resource, true));
			return basicDiagnostic;
		} else if (exception != null) {
			return new BasicDiagnostic(Diagnostic.ERROR, "com.keliangtek.simulab.target.editor", 0,
	...
	public SimulabTargetFormPage simulabTargetFormPage;

	@Override
	protected void addPages() {
		simulabTargetFormPage = new SimulabTargetFormPage(this, "simulabTargetFormPage", "Simulab Target Form Page");
		try {
			int index = addPage(simulabTargetFormPage);
	...
	public void createPages() {
		...
		//
		// selectionViewer.setLabelProvider(new DecoratingColumLabelProvider(new
		// AdapterFactoryLabelProvider(adapterFactory), new
		// DiagnosticDecorator(editingDomain, selectionViewer,
		// TargetEditorPlugin.getPlugin().getDialogSettings())));
		...
		// adapterFactory);
		// new ColumnViewerInformationControlToolTipSupport(selectionViewer, new
		...
		// public void requestActivation() {
		// super.requestActivation();
		// setgetSite().getShell().getDisplay().asyncExec
		// (new Runnable() {
		// public void run() {
		// updateProblemIndication();
		// }
		// });CurrentViewerPane(this);
		// }
		// };
		// viewerPane.createControl(getContainer());
		//
		...
		// //
		// {
		// ViewerPane viewgetSite().getShell().getDisplay().asyncExec
		// (new Runnable() {
		// public void run() {
		// updateProblemIndication();
		// }
		...
		// {
		// ViewerPane viewerPane =
		// new ViewerPgetSite().getShell().getDisplay().asyncExec
		// (new Runnable() {
		// public void run() {
		// updateProblemIndication();
		// }
		// });ane(getSite().getPage(), TargetEditor.this) {
		// @Override
		// public Viewer createViewer(Composite composite) {
		...
		// AdapterFactoryContentProvider(adapterFactory));
		// treeViewer.setLabelProvider(new DecoratingColumLabelProvider(new
		// AdapterFactoryLabelProvider(adapterFactory), new
		// DiagnosticDecorator(editingDomain, treeViewer)));
		//
		// new AdapterFactoryTreeEditor(treeViewer.getTree(), adapterFactory);
		// new ColumnViewerInformationControlToolTipSupport(treeViewer, new
		// DiagnosticDecorator.EditingDomainLocationListener(editingDomain,
		// treeViewer));
		//
		...
		//
		// TableColumn objgetSite().getShell().getDisplay().asyncExec
		// (new Runnable() {
		// public void run() {
		// updateProblemIndication();
		// }
		// });ectColumn = new TableColumn(table, SWT.NONE);
		...
		// AdapterFactoryContentProvider(adapterFactory));
		// tableViewer.setLabelProvider(new DecoratingColumLabelProvider(new
		// AdapterFactoryLabelProvider(adapterFactory), new
		// DiagnosticDecorator(editingDomain, tableViewer,
		// TargetEditorPlugin.getPlugin().getDialogSettings())));
		//
		// new ColumnViewerInformationControlToolTipSupport(tableViewer, new
		// DiagnosticDecorator.EditingDomainLocationListener(editingDomain,
		// tableViewer));
		//
		...
		// selfColumn.setText(getString("_UI_SelfColumn_label"));
		// selfColumn.setRgetSite().getShell().getDisplay().asyncExec
		// (new Runnable() {
		// public void run() {
		// updateProblemIndication();
		// }
		...
		// AdapterFactoryLabelProvider(adapterFactory), new
		// DiagnosticDecorator(editingDomain, treeViewerWithColumns,
		// TargetEditorPlugin.getPlugin().getDialogSettings())));
		//
		// new
		...
		// if (!guard) {
		// guard =getSite().getShell().getDisplay().asyncExec
		// (new Runnable() {
		// public void run() {
		// updateProblemIndication();
		// }
				...
				public void createControl(Composite parent) {
					...
					contentOutlineViewer.setLabelProvider(new DecoratingColumLabelProvider(
							new AdapterFactoryLabelProvider(adapterFactory), new DiagnosticDecorator(editingDomain,
									contentOutlineViewer, TargetEditorPlugin.getPlugin().getDialogSettings())));
	...
	public IPropertySheetPage getPropertySheetPage() {
		PropertySheetPage propertySheetPage = new ExtendedPropertySheetPage(editingDomain,
				ExtendedPropertySheetPage.Decoration.LIVE, TargetEditorPlugin.getPlugin().getDialogSettings()) {
			@Override
			public void setSelectionToViewer(List<?> selection) {
				SimulabTargetEditor.this.setSelectionToViewer(selection);
				SimulabTargetEditor.this.setFocus();
			}
	...
	public void doSaveAs() {
		...
		String[] files = TargetEditorAdvisor.openFilePathDialog(getSite().getShell(), SWT.SAVE, filters);
		if (files.length > 0) {
	...
	private static String getString(String key) {
		return TargetEditorPlugin.INSTANCE.getString(key);
	}
	...
	private static String getString(String key, Object s1) {
		return TargetEditorPlugin.INSTANCE.getString(key, new Object[] { s1 });
	}
